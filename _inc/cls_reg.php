...
class Cls_AdUser{
	...
	function mylist(){			
		...
		$sql .= ' and u_gid in (Select id from '.sh.'_group where typeid=0) order by id desc'; //只显示用户组的会员
	...
	function myform( $isedit ){
		...
		if ( $isedit ) {
			...
			$h = str_replace('{$fromunick}', $GLOBALS['main']->u_nick, $h);
			$h = $GLOBALS['main']->removemdbfield($h, sh.'_user');
		}
