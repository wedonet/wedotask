...
// end func
	...
	function fundel(){
		...
		/*提取这个分类和他的下级分类*/
		$sql = 'select id from `'.sh.'_class` where cid='.$this->c_class->cid; 
		...
		$classidlist = implode(',', $t);

		/*检测有没有这个分类下的记录*/
		$sql = 'select count(*) from `'.sh.'_task` where classid in('.$classidlist.')';
		$result = $this->main->execount($sql);
		if ($result>0){
			showerr('这个分类下还有记录,不能删除');
		}


		/* 删除此分类 */
        $sql = 'delete from `' . sh . '_class` where cid=' . $this->c_class->cid;;
        $sql .= ' and idpath like "' . $idpath . '%"';
